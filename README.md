...
The current release provides 45 MCP tools, read-only connectors for Google Search Console, GA4, Google Ads, HubSpot, Salesforce and generic CRM CSV data, deterministic keyword and campaign planning, live configurable dashboards, human-controlled local drafts, local stdio and a separately configured hosted Streamable HTTP reference edition.
...
This covers build, typecheck, deterministic decision cases, AI validation/fallback, local CSV behavior, mocked GSC/GA4/Google Ads/CRM responses, retry and error classification, normalization, action approval security, strategy and dashboard calculations, stdio discovery and invocation of all 45 tools.
